...
export interface LineBasedVirtualFile {
  uri: string;
  content: string;
  mappings: LineMapping[];
}

export class LineBasedVirtualFileManager {
  ...
  generateVirtualFile(
    ...
    for (let i = 0; i < components.length; i++) {
      ...
      const componentDirectives = directives.filter(d =>
        this.isDirectiveInComponent(d, component)
      );
   ...
   */
  mapHtmlToVirtual(htmlPosition: Position, mappings: LineMapping[]): LineMapping | null {
   ...
   */
  mapVirtualToHtml(virtualLine: number, mappings: LineMapping[]): LineMapping | null {
    return mappings.find(m => m.virtualLine === virtualLine) || null;
  }

  private extractScriptFunctions(document: TextDocument): string[] {
  ...
  private convertAlpineToTypeScript(expression: string, directiveName: string): string {
  ...
  private isDirectiveInComponent(
